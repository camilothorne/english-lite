...
%	:- module(readLine-mine,[readLine/1]).
...
readLine(String,Wordlist):-
   string_to_list(String,Words),	
   readWords(Words, Res),	
   checkWords(Res, Wordlist).
...
readWords([Char|Rest],[Word|L]):-
   readWord(Char,Rest,State),
   %append(Word,L,LL),
   name(Word,[Char]),
   readRest(Rest,L,State).

readRest([],_,ended).

readRest(Rest,L,notended):-
   readWords(Rest,L).
...
readWord(46,[],ended):-!.     %%% full stop

readWord(10,[],ended):-!.        %%% return

readWord(Code,[Code|Rest],State):-
   readWord(Code,Rest,State).

readWord(_,_,notended):-!.
